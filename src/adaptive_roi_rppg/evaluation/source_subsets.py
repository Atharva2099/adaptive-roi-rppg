...
def read_task_subset(root: str | Path, task_index: int, *, task_count: int = TASK_COUNT) -> tuple[Mapping[str, Any], list[dict[str, str]]]:
    if task_count != TASK_COUNT or not 0 <= task_index < task_count: _fail("invalid task index/count")
    directory = Path(root); index = _json(directory / "index.json")
    if index.get("schema") != "mcd-repeated-correction-beam-subsets-v1" or index.get("task_count") != TASK_COUNT: _fail("subset index schema is invalid")
    entries = index.get("tasks", []); entry = next((item for item in entries if item.get("task_index") == task_index), None)
    path = directory / f"task-{task_index:03d}.csv"
...
__all__ = ["TASK_COUNT", "SOURCE_FIELDS", "audit_plan_body_sha256", "validate_failure_audit_contract", "prepare_task_subsets", "read_task_subset", "stream_source_selection"]
