...
def read_frozen_selection(directory: str | Path, *, config_sha256: str, source_sha256: str, source_manifest_sha256: str) -> tuple[dict[str, Any], ...]:
    ...
    for index, row in enumerate(rows, 1):
        ...
        if not parsed["clip_id"] or not parsed["subject_id"] or parsed["selection_rank"] != index or not all(math.isfinite(parsed[field]) for field in ("seed0_mae_bpm", "seed1_mae_bpm", "seed2_mae_bpm", "three_seed_mean_mae_bpm")) or parsed["three_seed_mean_mae_bpm"] != sum(parsed[field] for field in ("seed0_mae_bpm", "seed1_mae_bpm", "seed2_mae_bpm")) / 3.0:
