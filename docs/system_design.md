...
Fail closed for unknown ROI order, missing fields, hash mismatch, duplicate keys, non-monotonic time, FPS mismatch, split overlap, non-MCD training manifests, GT in inference input, illegal cached teacher sequences, incomplete manifests, or extra/missing checkpoint files. For ordinary artifacts, write atomically: temporary file, validation, hash, then rename. Gate 5 is the explicit publication exception: create the final directory exclusively, write exclusive `STARTED.json`, write and validate the six substantive files, then write exclusive `COMPLETE.json` last; caught failures write exclusive `FAILED.json` best-effort. No Gate 5 directory rename, relocation, overwrite, or final-directory deletion is permitted. Completed manifests are immutable. Resumption validates every completed shard rather than trusting file existence.

## Implementation order and gates

Gate 1 contracts and validators are complete. Gate 2 passed independent review and full-data MCD acceptance on Polaris job `46838`. Gate 3 and Gate 4 are currently accepted under the repaired-reader authority of Polaris job `47260`. Gate 5 is accepted under Polaris job `47300`. The implementation order remains fixed:
...
Gate 5 is accepted as the frozen train-only evaluator: authenticated `gt_ppg` labels use `causal_periodogram_peak_no_subharmonic_v3`; joins require exact `(dataset_id, clip_id, hop_idx)` keys and hop times; scoring uses post-update belief HR, arithmetic clip means, and arithmetic subject clip means. The accepted Polaris evidence is job `47300`, snapshot SHA `5330ae9c25b7ff88468fe2f2cefae3fc2b2041753ea448aabcea5bf49e470de4`, wrapper SHA `1cd6b6b751d92de475bc86dfee3bc3343df7e96ac904993fb4f65df5f1176359`, and plan `frozen-plan-023260d9b7cb6c75b07f9e9856ecb85d425582c50c19b2b2da016b49100b9338`. The run covered 12 MCD train clips, 2 subjects, and 2,059 hops; equal-clip mean MAE was `6.494966476398912`, with 37 invalid selected measurements and zero overrides. The six substantive output files plus the two marker files were verified, including repeat substantive-output equality, source-bound recomputation, tampered-source failure, and existing-destination preservation. The publication protocol creates the final directory exclusively after its parent exists, writes exclusive `STARTED.json`, then the six substantive files, validates them, and writes exclusive `COMPLETE.json` last. Caught failures write exclusive `FAILED.json` best-effort. Readers accept only the exact marker/file state. No rename, relocation, overwrite, or final-directory deletion is permitted. Gate 6 historical parity replay is next. No training is allowed until Gate 6 passes.
