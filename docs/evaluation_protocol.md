...
Gate 5 freezes the deterministic full-face MCD train smoke with null seed and checkpoint. It uses the authenticated `causal_periodogram_peak_no_subharmonic_v3` labels, post-update belief HR, equal clip weighting, subject clip means, and no bootstrap. Publication uses an exclusive destination directory and separate exclusive state markers because the target NFSv3 rejected the prior directory-rename publication path. Existing destinations are refused untouched. Success requires six substantive files plus `STARTED.json` and last-written `COMPLETE.json`; caught failure writes `FAILED.json` best-effort, and interrupted or failed directories are never accepted.
