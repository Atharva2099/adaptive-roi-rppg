...
2. Reproduce the official OpenCV method as a separate preprocessing ruler,
   ...
   method.
3. On MCD only, design a causal tracker using full-frame MediaPipe detection,
   current-frame optical-flow landmark tracking, forward/backward consistency,
   periodic redetection, and, if needed, a separately justified
   profile-capable initializer.
4. For the proposed MCD-only tracker, require visible-skin and geometry
   validity and fail when those checks are not satisfied. Freeze that method
   on MCD before any held-out run.
5. Rerun all MMPD clips once, then report coverage, failure categories, and HR
