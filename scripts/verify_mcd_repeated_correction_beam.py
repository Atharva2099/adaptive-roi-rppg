#!/usr/bin/env python3
"""Publish one authenticated, deterministic MCD repeated-beam smoke shard.

This is deliberately smoke-only: `--smoke` evaluates exactly seed 0 and one
frozen-selection clip.  It cannot submit or imply a full experiment.
"""
...
from adaptive_roi_rppg.evaluation.source_subsets import TASK_COUNT, read_task_subset
...
def _args() -> argparse.Namespace:
    ...
    parser.add_argument("--smoke", action="store_true", help="required: one selection clip, seed 0 only")
    parser.add_argument("--output-dir", required=True); parser.add_argument("--selection-dir", required=True)
...
def _config() -> Mapping[str, Any]:
    path = Path(__file__).resolve().parents[1] / "configs/evaluation/mcd_repeated_correction_beam_v1.json"
    try: value = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc: raise ContractValidationError("beam config cannot be read") from exc
    expected = {"schema", "dataset_id", "selection", "window_hops", "primary_width", "sensitivity_widths", "ranking", "retention", "scope"}
    if set(value) != expected or value["schema"] != SCHEMA or value["dataset_id"] != "mcd" or value["selection"] != {"source":"frozen_worst54_mcd_replay_advantage_ppo", "clip_count":54, "seeds":[0,1,2]} or value["window_hops"] != 15 or value["primary_width"] != 8 or value["sensitivity_widths"] != list(SENSITIVITY_WIDTHS): _fail("beam config contract is invalid")
    return value
...
def _resolve_task(args: argparse.Namespace, selection_dir: str | Path) -> tuple[Mapping[str, Any], list[dict[str, str]], tuple[str, int], tuple[dict[str, Any], ...]]:
    ...
    selection = read_frozen_selection(
        selection_dir,
        config_sha256=hashlib.sha256(canonical_json_bytes(base_config)).hexdigest(),
        source_sha256=task_entry["_source_sha256"],
        source_manifest_sha256=task_entry["_plan_sha256"],
    )
    ...
    if task_entry["clip_id"] != pair[0] or int(task_entry["seed"]) != pair[1]:
        _fail("task subset does not match frozen clip/seed mapping")
    if args.smoke and (args.task_index != 0 or pair != (selection[0]["clip_id"], 0)):
        _fail("smoke requires task 0, the first frozen clip at seed 0")
    return task_entry, subset_rows, pair, selection


def main() -> int:
    args = _args()
    started = time.monotonic()
    if args.task_count != TASK_COUNT or not 0 <= args.task_index < TASK_COUNT: _fail("task count must be exactly 162")
    config = _config()
    task_entry, subset_rows, pair, selection = _resolve_task(args, args.selection_dir)
    ...
    selected = tuple(row for row in selection if (row["clip_id"], pair[1]) == pair)
    run_seed = pair[1]
    if not selected or task_entry["clip_id"] != pair[0] or int(task_entry["seed"]) != run_seed or task_entry["subject_id"] != selected[0]["subject_id"]: _fail("task subset is not the assigned frozen clip/seed")
    ...
    bundle = load_mcd_manifest_tree(args.manifest_tree); by_clip = {clip.clip_id: clip for clip in bundle.clip_manifests}; chosen = selected[0]; clip = by_clip.get(chosen["clip_id"])
    ...
    try:
        ...
        (root / "summary.json").write_bytes(canonical_json_bytes({"schema":SCHEMA, "scope":config["scope"], "task_index":args.task_index, "task_count":args.task_count, "clip_id":clip.clip_id, "subject_id":clip.subject_id, "seed":run_seed, "job_id":args.job_id, "node":args.node, "elapsed_seconds":round(time.monotonic() - started, 3), "eligible_anchor_count":len(anchors), "row_counts":{"factual_hops":len(factual), "one_time_hops":len(one_hops), "one_time_scores":len(one_scores), "beam_hops":len(hops), "beam_stats":len(stats)}, "widths":list(SENSITIVITY_WIDTHS), "parity_pass":True, "source_subset":{"sha256":task_entry["sha256"], "bytes":task_entry["bytes"]}}))
