...
from adaptive_roi_rppg.evaluation.long_term_regret import csv_bytes, replay_one_time_interventions, score_branches, validate_replay_parity
...
def _runtime_checkpoints(plan: Any, root_text: str, accepted_hashes: Mapping[int, str]) -> dict[int, Mapping[str, Any]]:
    ...
    for spec in plan.checkpoints:
        ...
        if root.resolve() not in candidate.parents or candidate.is_symlink() or not candidate.is_file(): _fail("checkpoint package path is invalid")
...
def _args() -> argparse.Namespace:
    ...
    # Full-cohort workers are the only supported execution mode.
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--source-subsets-dir", required=True)
    parser.add_argument("--beam-config", default=str(Path(__file__).resolve().parents[1] / "configs/evaluation/mcd_repeated_correction_beam_full_v1.json"))
    ...
    parser.add_argument("--task-index", type=int, default=0); parser.add_argument("--task-count", type=int, default=FULL_TASK_COUNT)
...
def _config(path: str | Path) -> Mapping[str, Any]:
    ...
    valid = (set(value) == full and value.get("schema") == "mcd-repeated-correction-beam-full-v1" and value.get("source_type") == "full_frozen_eval" and value.get("clip_count") == 533 and
                 value.get("subject_count") == 89 and value.get("seeds") == [0, 1, 2] and value.get("task_count") == FULL_TASK_COUNT and
                 value.get("task_mapping") == "task_index = 3 * source_plan_clip_position + seed" and
                 value.get("ranking") == "offline_gt_immediate_post_update_abs_error_then_action_index" and
                 value.get("retention") == "cumulative_abs_error_then_endpoint_abs_error_then_executed_sequence_then_origin_sequence")
...
def main() -> int:
    ...
    if args.task_count != FULL_TASK_COUNT or not 0 <= args.task_index < FULL_TASK_COUNT: _fail("task count does not match beam config")
