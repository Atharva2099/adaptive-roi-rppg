"""Run or merge the canonical MCD-train-only Gate 7 Oracle B/C evaluation."""
...
from adaptive_roi_rppg.evaluation.oracles import ORACLE_REPORT_SCHEMA, ORACLE_SCHEMA, ROW_FIELDS, aggregate_report, evaluate_oracle_shard, validate_aggregation
from adaptive_roi_rppg.labels.mcd import GT_RULE_ID, read_mcd_labels
...
class _TeacherExclusion(NamedTuple):
    """One immutable, canonical exclusion from the Gate 7 teacher cohort."""

    clip_id: str
    expected_invalid_label_count: int
    invalid_reason: str


# Historical MCD label pathology, frozen for Gate 7 provenance.  The order is
# part of the plan payload and therefore part of the plan ID.
TEACHER_EXCLUSION_CONTRACT = (
    _TeacherExclusion("7412_FullHDwebcam_after", 171, "degenerate_variation"),
    _TeacherExclusion("7412_IriunWebcam_after", 171, "degenerate_variation"),
    _TeacherExclusion("7412_USBVideo_after", 171, "degenerate_variation"),
)
...
def _require_slurm():
    ...
    if not job or not node:
        _fail("Gate 7 requires Slurm")
    return job, node


def _exclusion_payload(record: _TeacherExclusion) -> dict:
    return {"clip_id": record.clip_id,
            "expected_invalid_label_count": record.expected_invalid_label_count,
            "invalid_reason": record.invalid_reason}


def _validate_declared_exclusion(args, bundle, clip, record: _TeacherExclusion) -> None:
    labels = read_mcd_labels(bundle, args.gt_root, record.clip_id, "train", clip)
    if len(labels) != record.expected_invalid_label_count:
        _fail(f"Gate 7 declared exclusion {record.clip_id} does not have exactly {record.expected_invalid_label_count} labels")
    for label in labels:
        if (label.clip_id != record.clip_id or label.valid or label.gt_hr_bpm is not None
                or label.invalid_reason != record.invalid_reason):
            _fail(f"Gate 7 declared exclusion {record.clip_id} labels do not match the frozen invalid-label contract")
...
def _plan(args):
    ...
    if (len(train_ids) != 3060 or len(eval_ids) != 540
        ...
        _fail("Gate 7 requires 3060 train clips and 510 train subjects, plus 540 held-out eval clips across 90 eval subjects, with no overlap")
    train_by_id = {clip.clip_id: clip for clip in manifest_train_clips}
    if len(train_by_id) != len(manifest_train_clips):
        _fail("Gate 7 train clip IDs must be unique")
    for record in TEACHER_EXCLUSION_CONTRACT:
        clip = train_by_id.get(record.clip_id)
        if clip is None:
            _fail(f"Gate 7 declared exclusion is not a train clip: {record.clip_id}")
        _validate_declared_exclusion(args, bundle, clip, record)
    excluded_ids = {record.clip_id for record in TEACHER_EXCLUSION_CONTRACT}
    clips = tuple(clip for clip in manifest_train_clips if clip.clip_id not in excluded_ids)
    subjects = sorted({clip.subject_id for clip in clips})
    if len(clips) != 3057 or len(subjects) != 510:
        _fail("Gate 7 requires exactly 3057 eligible teacher clips across all 510 train subjects")
    ...
    payload = {"schema": "gate7-mcd-oracle-plan-v3", "dataset_id": split.dataset_id,
               ...
               "source_inventory_sha256": sha256_file(inventory_path), "split": "train",
               ...
               "eligible_teacher_counts": {"train_subject_count": len(subjects), "train_clip_count": len(clips)},
               "teacher_exclusions": [_exclusion_payload(record) for record in TEACHER_EXCLUSION_CONTRACT],
    ...
    payload["plan_id"] = "gate7-plan-" + hashlib.sha256(canonical_json_bytes(payload)).hexdigest()
...
def _load_and_evaluate_bindings(args, bindings):
    """Load only authenticated canonical train sources and recompute Oracle B/C."""
    bundle = load_mcd_manifest_tree(args.manifest_tree)
    ...
    for binding in bindings:
        ...
        if clip != binding:
            _fail("canonical manifest no longer matches exact Gate 7 binding")
        frames[binding.clip_id] = build_pos_measurements(
            read_mcd_canonical_frames(bundle, args.state_root, binding.clip_id, "train"))
        labels[binding.clip_id] = read_mcd_labels(bundle, args.gt_root, binding.clip_id, "train", clip)
        metadata[binding.clip_id] = {"subject_id": binding.subject_id, "view": binding.view,
                                      "condition": binding.condition}
    return evaluate_oracle_shard(frames, labels, metadata, beam_width=args.beam_width)
