#!/usr/bin/env python3
"""Aggregate the completed job-48193 MCD repeated-correction diagnostic."""
...
from adaptive_roi_rppg.evaluation.long_term_regret import partition_clip_seed_tasks, read_frozen_selection, subject_block_bootstrap
from adaptive_roi_rppg.evaluation.repeated_correction_beam import PRIMARY_WIDTH, SCHEMA, SENSITIVITY_WIDTHS, WINDOW
from adaptive_roi_rppg.evaluation.source_subsets import FULL_TASK_COUNT, TASK_COUNT, read_task_subset
try:
    from verify_mcd_long_term_regret import _load_config
except ModuleNotFoundError:  # pragma: no cover - import context only
    from scripts.verify_mcd_long_term_regret import _load_config
SEEDS = (0, 1, 2)
...
def _beam(hops: Sequence[Mapping[str, Any]], stats: Sequence[Mapping[str, Any]], factual: Mapping[int, Mapping[str, Any]]) -> dict[int, dict[str, Any]]:
    ...
    for key, (factual_path, retained) in by_width.items():
        ...
        best, final = min(retained, key=_path_key), rows[-1]
...
def _estimate(rows: Sequence[Mapping[str, Any]], key: str) -> dict[str, Any]:
    ...
    result = subject_block_bootstrap(adapted)
...
def _beam_config(path: str | Path) -> Mapping[str, Any]:
    ...
    if value.get("schema") == SCHEMA:
        if value.get("dataset_id") != "mcd" or value.get("selection", {}).get("clip_count") != 54:
            _fail("worst-54 beam config is invalid")
...
def analyze_results(results_root: str | Path, output_dir: str | Path, *, job_id: str | int | Sequence[str | int], source_subsets_dir: str | Path,
                    selection_dir: str | Path | None = None, beam_config: str | Path | None = None,
    ...
    config = _beam_config(beam_config or Path(__file__).resolve().parents[1] / "configs/evaluation/mcd_repeated_correction_beam_v1.json")
    full = config["schema"] == "mcd-repeated-correction-beam-full-v1"
    ...
    selection_root = Path(selection_dir) if selection_dir else None
    inputs = (results, subsets) if selection_root is None else (results, selection_root, subsets)
    if any(path.is_symlink() or not path.is_dir() for path in inputs): _fail("input roots must be real directories")
    if not full and selection_root is None:
        _fail("worst-54 aggregation requires frozen selection")
    ...
    task_count = FULL_TASK_COUNT if full else TASK_COUNT
    index_schema = "mcd-repeated-correction-beam-full-subsets-v1" if full else "mcd-repeated-correction-beam-subsets-v1"
    ...
    if full:
        ...
        legacy_config_hash = hashlib.sha256(canonical_json_bytes(_load_config())).hexdigest()
        selection = read_frozen_selection(selection_root, config_sha256=legacy_config_hash, source_sha256=index["source_sha256"], source_manifest_sha256=index["plan_sha256"])
        subjects = {row["clip_id"]: row["subject_id"] for row in selection}
        task_entries = {item["task_index"]: item for item in entries}
    run_prefix = "mcd-repeated-beam-full" if full else "mcd-repeated-beam"
    ...
    for task in expected_tasks:
        if full:
            ...
            pair = partition_clip_seed_tasks(selection, task_index=task, task_count=TASK_COUNT)
            if len(pair) != 1: _fail("selection does not map one pair per task")
            clip, seed = pair[0]
        ...
        if full and (summary.get("beam_config_schema"), summary.get("beam_config_sha256")) != (config["schema"], beam_config_hash):
    ...
    report = {"schema": "mcd-repeated-beam-behavior-v2", "job_ids": list(job_ids),
              ...
              "provenance": {"beam_config_sha256": beam_config_hash, "selection_sha256": None if full else sha256_file(selection_root / "clip_selection.csv"),
...
def main() -> int:
    ...
    parser.add_argument("--beam-config")
    parser.add_argument("--seeds", type=int, nargs="+", help="seed subset to aggregate; defaults to every seed in the beam config")
    args = parser.parse_args()
    analyze_results(results_root=args.results_root, output_dir=args.output_dir, job_id=args.job_id or ["48193"],
                    selection_dir=args.selection_dir, source_subsets_dir=args.source_subsets_dir, beam_config=args.beam_config,
                    seeds=args.seeds)
    return 0
