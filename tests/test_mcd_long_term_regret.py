...
class LongTermRegretTests(unittest.TestCase):
    ...
    def test_full_beam_verifier_boundary_mapping(self):
        path = Path(__file__).resolve().parents[1] / "scripts/verify_mcd_repeated_correction_beam.py"
        spec = importlib.util.spec_from_file_location("verify_mcd_repeated_correction_beam_full_test", path)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        with patch.dict(sys.modules, {"verify_mcd_long_term_regret": importlib.import_module("scripts.verify_mcd_long_term_regret")}):
            spec.loader.exec_module(module)
        config = {"schema": "mcd-repeated-correction-beam-full-v1"}
        for task, expected in ((0, (0, 0)), (1, (0, 1)), (2, (0, 2)), (1596, (532, 0)), (1597, (532, 1)), (1598, (532, 2))):
            entry = {"task_index": task, "clip_position": expected[0], "clip_id": f"clip-{expected[0]}", "subject_id": f"subject-{expected[0]}", "seed": expected[1]}
            args = SimpleNamespace(source_subsets_dir="unused", task_index=task, task_count=1599, selection_dir=None, smoke=False)
            with patch.object(module, "read_task_subset", return_value=(entry, [])):
                _entry, _rows, pair, chosen = module._resolve_task(args, config)
            self.assertEqual(pair, (f"clip-{expected[0]}", expected[1]))
            self.assertEqual(chosen["subject_id"], f"subject-{expected[0]}")
    ...
    def test_frozen_selection_accepts_float_round_trip_but_rejects_wrong_mean(self):
        rows = [{"clip_id": f"clip-{index:03d}", "subject_id": f"subject-{index % 89:03d}", "seed0_mae_bpm": float(index), "seed1_mae_bpm": float(index), "seed2_mae_bpm": float(index), "three_seed_mean_mae_bpm": float(index), "selection_rank": index + 1} for index in range(54)]
        rows[0].update(seed0_mae_bpm=0.1, seed1_mae_bpm=0.2, seed2_mae_bpm=0.3, three_seed_mean_mae_bpm=0.2)
        with TemporaryDirectory() as tmp:
            root = Path(tmp) / "selection"; root.mkdir()

            def write_selection(selection_rows):
                raw = csv_bytes(selection_rows, CLIP_SELECTION_FIELDS)
                (root / "clip_selection.csv").write_bytes(raw)
                provenance = {"schema": SELECTION_SCHEMA, "config_sha256": "c" * 64, "source_sha256": "p" * 64, "source_manifest_sha256": "m" * 64, "selection_sha256": hashlib.sha256(raw).hexdigest(), "clip_count": 54, "selection_rule": "mean over Advantage PPO seeds 0,1,2; descending MAE then clip_id tie-break"}
                (root / "selection_provenance.json").write_text(json.dumps(provenance))
            ...
            self.assertEqual(len(read_frozen_selection(root, config_sha256="c" * 64, source_sha256="p" * 64, source_manifest_sha256="m" * 64)), 54)
            rows[0]["three_seed_mean_mae_bpm"] = 0.21
            write_selection(rows)
            with self.assertRaises(ContractValidationError):
                read_frozen_selection(root, config_sha256="c" * 64, source_sha256="p" * 64, source_manifest_sha256="m" * 64)
