import csv, io, json, tempfile, unittest, hashlib, os
...
from adaptive_roi_rppg.evaluation.adapters.mmpd.publication import HOP_FIELDS, _exclusive, rebuild_gate9_artifacts, start_gate9_evaluation, publish_gate9_evaluation, fail_gate9_evaluation, validate_gate9_evaluation_tree
...
class Gate9LifecycleTests(unittest.TestCase):
    def test_output_parent_must_be_existing_real_directory(self):
        with tempfile.TemporaryDirectory() as d:
            base = Path(d); missing = base / "missing" / "out"; link = base / "link"; link.symlink_to(base, target_is_directory=True)
            for destination in (missing, link / "out"):
                with self.subTest(destination=destination), self.assertRaises(ContractValidationError): start_gate9_evaluation(destination, {})
    ...
    def test_exclusive_write_removes_member_after_close_failure(self):
        with tempfile.TemporaryDirectory() as d:
            path = Path(d) / "COMPLETE.json"
            with patch("adaptive_roi_rppg.evaluation.adapters.mmpd.publication.os.fsync", side_effect=OSError("fsync")):
                with self.assertRaises(OSError): _exclusive(path, b"corrupt")
